...
import com.ilnodstudio.ansartelecom.screen.list.elektronics.SmartPhon.SmartData

class SmartAdapter(private val context: Context) : RecyclerView.Adapter<SmartAdapter.HomeHolder>() {

    class HomeHolder(val binding: MobitemBinding): RecyclerView.ViewHolder(binding.root)

    var productList: List<SmartData> = emptyList()




    var onItemClick: ((SmartData)->Unit)? = null
    ...
    fun setData(data: List<SmartData>) {
        productList = data
