...
class AdminAdapter(private val context: Context) : RecyclerView.Adapter<AdminAdapter.HomeHolder>() {

    class HomeHolder(val binding: MobitemBinding): RecyclerView.ViewHolder(binding.root)

    var productList: List<AdminData> = emptyList()




    var onItemClick: ((AdminData)->Unit)? = null
    ...
    fun setData(data: List<AdminData>) {
        productList = data
    ...
    override fun onBindViewHolder(holder: HomeHolder, position: Int) {
        ...
        if (data.imgUrlList.isNotEmpty()) {
            ...
            Glide.with(context)
                .load(R.drawable.home_sim) // Заглушка или пустое изображение
                .apply(RequestOptions().override(512, 512))
                .into(holder.binding.imageView)
        }
